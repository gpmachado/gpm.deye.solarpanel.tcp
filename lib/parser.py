"""
Parameter parser for Solarman inverter register definitions.
Adapted from home_assistant_solarman (MIT License).
Original: https://github.com/StephanJoubert/home_assistant_solarman
"""
...
class ParameterParser:
    ...
    def try_parse_version(self, rawData, definition, start, length):
        ...
        for r in definition['registers']:
            ...
            if (index >= 0) and (index < length):
                temp = rawData[index]
                value = (value + str(temp >> 12) + "." + str(temp >> 8 & 0x0F)
                         + "." + str(temp >> 4 & 0x0F) + "." + str(temp & 0x0F))
            else:
                found = False
        if found:
            self.result[title] = value
    ...
    def try_parse_time(self, rawData, definition, start, length):
        ...
        for r in definition['registers']:
            ...
            if (index >= 0) and (index < length):
                temp = rawData[index]
                value = (str("{:02d}".format(int(temp / 100))) + ":"

...
/** @type {number} Backoff duration when offline at night (30 minutes) */
const NIGHT_BACKOFF_MS = 30 * 60 * 1000;

/**
 * Watts above which a daytime poll failure marks the device unavailable.
 * Below this threshold the inverter is considered idle (startup / cloud cover).
 */
const POWER_THRESHOLD_W = 5;

/**
 * How many consecutive daytime poll failures before marking unavailable.
 * Gives the inverter time to initialise at dawn without showing error icon.
 */
...
class DeyeDevice extends Device {
  ...
  async onInit() {
    this.log('DeyeDevice init v1.3:', this.getName());

    this._lastPowerW = 0;
    this._backoffUntil = 0;
    this._polling = false;
    this._failCount = 0;    // consecutive poll failures this production window
    this._wasNight = true; // assume night on start so _lastPowerW resets at dawn
   ...
   * Energy counters (meter_power.*) are intentionally preserved.
   *
  ...
  async _applyZeros() {
    const caps = [
      ...
      'measure_power.grid', 'measure_power.load',
   ...
   *
   * Morning / dawn behaviour:
   *   - Tracks night->day transition via `_wasNight`.
   *   - On the first poll after night, `_lastPowerW` and `_failCount` are
   *     reset so early failures during inverter boot don't immediately mark
   *     the device unavailable.
   *   - Only marks unavailable after FAIL_TOLERANCE consecutive failures
   *     while the inverter was known to be producing (> POWER_THRESHOLD_W).
   *
   * @returns {Promise<void>}
   */
  async _poll() {
    ...
    // -- Dawn transition: reset state so morning failures are tolerated -------
    if (this._wasNight && !isNight) {
      this.log('poll: dawn detected -- resetting power state');
      this._lastPowerW = 0;
      this._failCount = 0;
    }
    ...
    try {
      ...
      // Successful poll -- reset failure counter and mark available
      this._failCount = 0;
      ...
      for (const def of this._definitions) {
        ...
        // Track last known AC output power for failure classification
      ...
      if (isNight) {
        // -- Night: expected offline -- zero live values, back off 30 min ------
        await this._applyZeros();
        this.setAvailable().catch(() => { });
        this.log(`poll: night offline (expected) -- backing off 30 min: ${err.message}`);
        this._backoffUntil = Date.now() + NIGHT_BACKOFF_MS;

      } else if (this._lastPowerW > POWER_THRESHOLD_W && this._failCount >= FAIL_TOLERANCE) {
        // -- Day + was producing + repeated failures = real problem ------------
        this.error(`Poll failed ${this._failCount}x while active (${this._lastPowerW}W): ${err.message}`);
        this.setUnavailable(
          this.homey.__('device.unavailable', { error: err.message }),
        ).catch(() => { });

      } else {
        // -- Day + idle OR within tolerance window (dawn startup) --------------
        // Don't mark unavailable -- inverter may still be initialising.
        this.log(
          `Poll failed (attempt ${this._failCount}/${FAIL_TOLERANCE}, ` +
          `lastPower=${this._lastPowerW}W, night=${isNight}): ${err.message}`,
        );
      }
    } finally {
      this._client.disconnect();

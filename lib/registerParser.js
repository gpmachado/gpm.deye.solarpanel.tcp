...
function decodeRegister(def, registerMap) {
  // Derived fields (rule: 'sum') are resolved in readAll() -- not here.
  if (def.rule === 'sum') return null;
 ...
 *
 * If `definitions` has a companion `_requests` array (explicit Modbus blocks,
 * e.g. from a YAML with non-contiguous address ranges), those blocks are used
 * directly instead of the auto-computed merged ranges from buildReadRequests.
 * This avoids reading large gaps between blocks that the inverter may reject.
 *
 * Usage with explicit blocks:
 *   const defs = [...];
 *   defs._requests = [{ start: 0x0003, end: 0x0059, mbFunctionCode: 0x03 }, ...];
 *
 * @param {import('./solarmanV5').SolarmanV5Client} client
 * @param {RegisterDefinition[]} definitions
...
async function readAll(client, definitions) {
  ...
  if (definitions._requests && definitions._requests.length > 0) {
    // Convert explicit {start, end, mbFunctionCode} blocks to {fc, start, count}
    requests = definitions._requests.map(r => ({
      fc:    r.mbFunctionCode ?? 0x03,
      start: r.start,
      count: r.end - r.start + 1,
    }));
  } else {
    requests = buildReadRequests(definitions);
  }
  ...
  const result = {};
  for (const def of definitions) {
    if (def.rule === 'sum') continue; // resolved in second pass
    result[def.name] = decodeRegister(def, registerMap);
  }

  // Second pass: derived fields (rule: 'sum')
  // Example: { name: 'Total AC Power', rule: 'sum', sources: ['Inverter L1 Power', ...], homeyCapability: 'measure_power' }
  for (const def of definitions) {
    if (def.rule !== 'sum') continue;
    let total = 0;
    let valid = true;
    for (const src of def.sources) {
      const v = result[src];
      if (typeof v !== 'number') { valid = false; break; }
      total += v;
    }
    result[def.name] = valid ? total : null;
  }
